...
//---------------------------Start-------------------------------code by Kenneth Segura------
	...
	while (l <= r) {
		...
		if (items[m].getName().equals(name)) return items[m];
		
		// If m greater, ignore left half
		if (items[m].getName().compareTo(name) < 0) l = m + 1;
		
		// If m is smaller, ignore right half
		else r = m - 1;
	}
	
	// if at this point we have not found the element return null
	return null;
    }
...
//---------------------------------------------------------------------------------------------    
	...
	for (int i = 1; i < numberOfItems; i++) {
		...
		Movie yr = items[i];
		int j = i - 1;
		while (j >= 0 && items[j].getYear() > yr.getYear()) {
			items[j + 1] = items[j];
			j = j - 1;
...
//----------------------------------------------------------------------------------------------
	...
	//Sets up outer loop for selection sort to move through array
...
//-----------------------------------------------------------------------------------------------
    ...
    public static Movie searchMovieByName (String name, Movie[] movies, int l, int r) {
		...
		//checks if left index is what we are looking for
		if (movies[l].getName().equals(name)) {
			return movies[l];

		}
		// if not recur moving left index up 1
